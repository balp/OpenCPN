...
void s52plib::UpdateMarinerParams( void ) {
      ...
      //      Symbol Style
      if( SIMPLIFIED == m_nSymbolStyle ) S52_setMarinerParam(
                  S52_MAR_SYMPLIFIED_PNT, 1.0 );
      else S52_setMarinerParam( S52_MAR_SYMPLIFIED_PNT, 0.0 );

      //      Boundary Style
      if( SYMBOLIZED_BOUNDARIES == m_nBoundaryStyle ) S52_setMarinerParam(
                  S52_MAR_SYMBOLIZED_BND, 1.0 );
      else S52_setMarinerParam( S52_MAR_SYMBOLIZED_BND, 0.0 );
...
LUPrec *s52plib::FindBestLUP( wxArrayPtrVoid *nameMatch, char *objAtt,
      ...
      for( i = 0; i < nLUPCandidates; ++i ) {
            ...
            for( unsigned int iLUPAtt = 0;
                        iLUPAtt < LUPCandidate->ATTCArray->GetCount(); iLUPAtt++ ) {
      ...
      if( pR ) {
            ...
            if( pR->pixelPtr ) {
                  if( pR->parm0 == ID_GL_PATT_SPEC ) {
                        render_canvas_parms *pp =
                                    (render_canvas_parms *) ( pR->pixelPtr );
                        free( pp->pix_buff );
                        delete pp;
...
//            PRINTF("ERROR: chopping attribut value !? \n");
            ...
            if( ( !strncmp( buf, "VERCLR", 6 ) )
                        || ( !strncmp( buf, "VERCCL", 6 ) ) ) {
            ...
            if( !strncmp( buf, "NATSUR", 6 ) ) {
                  ...
                  while( tkz.HasMoreTokens() ) {
                        ...
                        wxString nat = rzRules->chart->GetAttributeDecode( natsur_att,
                                    i );
                  ...
                  if( ptext->m_pRGBA ) {
                        ...
                        if( bdraw ) {
                              ...
                              if( ( draw_width > 0 ) && ( draw_height > 0 ) ) {
                                    ...
                                    glDrawPixels( draw_width, draw_height, GL_RGBA,
                                                GL_UNSIGNED_BYTE, ptext->m_pRGBA );
                                    glPixelZoom( 1, 1 );
                                    glDisable( GL_BLEND );
...
bool s52plib::CheckTextRectList( const wxRect &test_rect, S57Obj *pobj ) {
      ...
      for( ObjList::Node *node = m_textObjList.GetFirst(); node;
                  node = node->GetNext() ) {
...
bool s52plib::TextRenderCheck( ObjRazRules *rzRules ) {
      ...
      if( ( rzRules->chart->GetChartType() == CHART_TYPE_CM93 )
            ...
            else if( !strncmp( rzRules->obj->FeatureName, "SEAARE", 6 ) ) return false;
            else if( !strncmp( rzRules->obj->FeatureName, "LNDRGN", 6 ) ) return false;
      }

      return true;
}

int s52plib::RenderT_All( ObjRazRules *rzRules, Rules *rules, ViewPort *vp,
            bool bTX ) {
      ...
      if( text ) {
            ...
            wxPoint r;
            rzRules->chart->GetPointPix( rzRules, rzRules->obj->y, rzRules->obj->x,
                        &r );
...
//            if ( rzRules->obj->Primitive_type == GEO_POINT )
            {
                  ...
                  rzRules->chart->GetPixPoint( rect.GetX(),
                              rect.GetY() + rect.GetHeight(), &plat, &plon, vp );
                  bbtext.SetMin( plon, plat );

                  rzRules->chart->GetPixPoint( rect.GetX() + rect.GetWidth(),
                              rect.GetY(), &plat, &plon, vp );
                  bbtext.SetMax( plon, plat );

                  if( rzRules->obj->bBBObj_valid ) rzRules->obj->BBObj.Expand(
                              bbtext );
                  else {
...
wxImage s52plib::RuleXBMToImage( Rule *prule ) {
      ...
      for( int iy = 0; iy < height; iy++ ) {
            ...
            for( int ix = 0; ix < width; ix++ ) {
                  ...
                  if( cref >= 0 ) {
                        S52color *pthisbitcolor = (S52color *) ( pColorArray->Item(
                                    cref ) );
...
int s52plib::RenderSY( ObjRazRules *rzRules, Rules *rules, ViewPort *vp ) {
      ...
      if( rules->razRule != NULL ) {
            ...
            if( rules->razRule->definition.SYDF == 'V' ) RenderHPGL( rzRules,
                        rules->razRule, r, vp, angle );

            else if( rules->razRule->definition.SYDF == 'R' ) RenderRasterSymbol(
                        rzRules, rules->razRule, r, vp, angle );
            ...
            for( int iseg = 0; iseg < rzRules->obj->m_n_lsindex; iseg++ ) {
                  ...
                  if( ( inode >= 0 ) ) {
                        ...
                        if( pnode ) {
                              ...
                              easting = *ppt++;
                              northing = *ppt;
                              rzRules->chart->GetPointPix( rzRules, (float) northing,
                                          (float) easting, &pra );
                        }
                  ...
                  for( int ipc = istart; ipc < ndraw; ipc++ ) {
                        ...
                        ClipResult res = cohen_sutherland_line_clip_i( &x0, &y0, &x1,
                                    &y1, xmin_, xmax_, ymin_, ymax_ );
...
//                    pdc->DrawLines(npt + 1, ptp);
            ...
            PolyTrapGroup *pptg =
                        rzRules->obj->pPolyTrapGeo->Get_PolyTrapGroup_head();
...
//      Create a color for drawing adjustments outside of HPGL renderer
      ...
      if( rzRules->obj->m_n_lsindex ) {
            ...
            for( int iseg = 0; iseg < rzRules->obj->m_n_lsindex; iseg++ ) {
                  ...
                  if( ( inode >= 0 ) && ( jnode >= 0 ) ) draw_lc_poly( m_pdc, color,
                              w, ptp, nls + 2, sym_len, sym_factor, rules->razRule, vp );
                  else draw_lc_poly( m_pdc, color, w, &ptp[1], nls, sym_len,
                              sym_factor, rules->razRule, vp );
      ...
      else if( rzRules->obj->geoPt ) // if the object is not described by a poly structure
      {
            ...
            draw_lc_poly( m_pdc, color, w, ptp, npt, sym_len, sym_factor,
                        rules->razRule, vp );
...
void s52plib::draw_lc_poly( wxDC *pdc, wxColor &color, int width, wxPoint *ptp,
      ...
      if( pdc ) {
            wxPen *pthispen = wxThePenList->FindOrCreatePen( color, width,
                        wxSOLID );
            m_pdc->SetPen( *pthispen );
            ...
            for( int iseg = 0; iseg < npt - 1; iseg++ ) {
                  ...
                  if( seg_len >= 1.0 ) {
                        if( seg_len <= sym_len * sym_factor ) {
                              ...
                              pdc->DrawLine( (int) xs, (int) ys, ptp[iseg + 1].x,
                                          ptp[iseg + 1].y );
                        }
                  }
            } // for
...
int s52plib::RenderCARC( ObjRazRules *rzRules, Rules *rules, ViewPort *vp ) {
      ...
      if( ( rules->razRule->pixelPtr == NULL )
            ...
            if( fabs( sectr2 - sectr1 ) != 360 ) // not necessary for all-round lights
                  ...
                  wxPen *pblockpen = wxThePenList->FindOrCreatePen( *wxBLACK, 10,
                              wxSOLID );
                  ...
                  int x0 = ( width / 2 )
                              + (int) ( rad * cos( start_angle * PI / 180. ) );
                  int y0 = ( height / 2 )
                              - (int) ( rad * sin( start_angle * PI / 180. ) );
            ...
            if( m_pdc ) {
                  ...
                  wxPen *pthispen = wxThePenList->FindOrCreatePen( color,
                              outline_width, wxSOLID );
                  mdc.SetPen( *pthispen );
                  wxBrush *pthisbrush = wxTheBrushList->FindOrCreateBrush( color,
                              wxTRANSPARENT );
                  mdc.SetBrush( *pthisbrush );

                  mdc.DrawEllipticArc( width / 2 - rad, height / 2 - rad, rad * 2,
                              rad * 2, sb, se );
                  ...
                  if( arc_width ) {
                        ...
                        pthispen = wxThePenList->FindOrCreatePen( colorb, arc_width,
                                    wxSOLID );
                        mdc.SetPen( *pthispen );
...
//                  glColor4ub( colorb.Red(), colorb.Green(), colorb.Blue(), 255 );
      ...
      if( !m_pdc ) // opengl
      {
            ...
            wxMemoryDC mdc;
            mdc.SelectObject(
                        (wxBitmap &) ( *( (wxBitmap *) ( rules->razRule->pixelPtr ) ) ) );
      ...
      symbox.SetMin( plon, plat );
      rzRules->chart->GetPixPoint( r.x + rules->razRule->parm2 + b_width,
                  r.y + rules->razRule->parm3, &plat, &plon, vp );
      symbox.SetMax( plon, plat );
...
bool s52plib::PreloadOBJLFromCSV( wxString &csv_file ) {
      ...
      while( !file.Eof() ) {
            ...
            if( token.Len() ) {
                  ...
                  bool bdup = false;
                  for( unsigned int iPtr = 0; iPtr < pOBJLArray->GetCount();
                              iPtr++ ) {
                        OBJLElement *pOLEt =
                                    (OBJLElement *) ( pOBJLArray->Item( iPtr ) );
                        if( !token.CmpNoCase(
                                    wxString( pOLEt->OBJLName, wxConvUTF8 ) ) ) {
                              bdup = true;
                              break;
                        }
                  }
                  ...
                  if( !bdup ) {
                        OBJLElement *pOLE = (OBJLElement *) calloc( sizeof(OBJLElement),
                                    1 );
                  ...
                  if( ( iyp >= ybt ) && ( iyp < yt ) ) {
                        ...
                        if( ledge[iyp] != -1 ) {
                              ...
                              if( pPatt_spec ) // Pattern
                              {
                                    ...
                                    int patt_y = abs( ( iyp - pPatt_spec->y ) )
                                                % patt_size_y;
                                    ...
                                    while( ix <= ixm ) {
                                           ...
                                           */
                                          {
                                                ...
                                                if( alpha > 128 ) {
                                                      ...
                                                      unsigned char r = (unsigned char) ( pp[0]
                                                                  * da );
                                                      unsigned char g = (unsigned char) ( pp[1]
                                                                  * da );
                                                      unsigned char b = (unsigned char) ( pp[2]
                                                                  * da );
...
inline int s52plib::dda_trap( wxPoint *segs, int lseg, int rseg, int ytop,
            int ybot, S52color *c, render_canvas_parms *pb_spec,
            render_canvas_parms *pPatt_spec ) {
      ...
      if( obj->pPolyTessGeo ) {
            ...
            while( p_tp ) {
                  ...
                  if( BBView.GetMaxX() > 360. ) {
                        wxBoundingBox bbRight( 0., BBView.GetMinY(),
                                    BBView.GetMaxX() - 360., BBView.GetMaxY() );
                        if( bbRight.Intersect( *( p_tp->p_bbox ), margin ) != _OUT ) b_greenwich =
                                    true;
                  }

                  if( b_greenwich
                              || ( BBView.Intersect( *( p_tp->p_bbox ), margin ) != _OUT ) ) {
            ...
            if( obj->pPolyTrapGeo->IsOk() /*&& (obj->Index == 7) && ( obj->pPolyTrapGeo->GetnVertexMax() < 1000)*/) {
                  ...
                  wxPoint *ptp = (wxPoint *) malloc(
                              ( nVertex + 1 ) * sizeof(wxPoint) );

                  rzRules->chart->GetPointPix(
                              rzRules,
                              obj->pPolyTrapGeo->Get_PolyTrapGroup_head()->ptrapgroup_geom,
                              ptp, nVertex );
                  ...
                  for( int i = 0; i < ntraps; i++ ) {
                        ...
                        if( ( trap_y_top >= pb_spec->y - trap_height )
                                    && ( trap_y_bot
                                                <= pb_spec->y + pb_spec->height + trap_height ) ) {
                              /*
                               if(obj->Index == 7)
                               {
...
//                              if((lseg == 66) && (trap_y_top < 0))
                              //                                    cs.B = 128;
                              dda_trap( ptp, lseg, rseg, trap_y_top, trap_y_bot, cd,
                                          pb_spec, pPatt_spec );
...
int s52plib::RenderToGLAC( ObjRazRules *rzRules, Rules *rules, ViewPort *vp ) {
      ...
      if( rzRules->obj->pPolyTessGeo ) {
            ...
            wxPoint *ptp = (wxPoint *) malloc(
                        ( rzRules->obj->pPolyTessGeo->GetnVertexMax() + 1 )
                                    * sizeof(wxPoint) );
            ...
            while( p_tp ) {
                  ...
                  if( b_greenwich
                         ...
                         */
                        rzRules->chart->GetPointPix( rzRules,
                                    (wxPoint2DDouble*) p_tp->p_vertex, ptp, p_tp->nVert );
                        ...
                        switch ( p_tp->type ){
                              ...
                              case PTG_TRIANGLES: {
                                    for( int it = 0; it < p_tp->nVert; it += 3 ) {
                                          int xmin =
                                                      wxMin(ptp[it].x, wxMin(ptp[it+1].x, ptp[it+2].x));
                                          int xmax =
                                                      wxMax(ptp[it].x, wxMax(ptp[it+1].x, ptp[it+2].x));
                                          int ymin =
                                                      wxMin(ptp[it].y, wxMin(ptp[it+1].y, ptp[it+2].y));
                                          int ymax =
                                                      wxMax(ptp[it].y, wxMax(ptp[it+1].y, ptp[it+2].y));
                              ...
                              case PTG_TRIANGLES: {
                                    for( int it = 0; it < p_tp->nVert; it += 3 ) {
                                          ...
                                          if( rect.Intersects( m_render_rect ) ) {
                                                glBegin( GL_TRIANGLES );
                                                glVertex3f( ptp[it].x, ptp[it].y, z_clip_geom );
                                                glVertex3f( ptp[it + 1].x, ptp[it + 1].y,
                                                            z_clip_geom );
                                                glVertex3f( ptp[it + 2].x, ptp[it + 2].y,
                                                            z_clip_geom );
                                                glEnd();
                                          }
                                    }
...
render_canvas_parms* s52plib::CreatePatternBufferSpec( ObjRazRules *rzRules,
            Rules *rules, ViewPort *vp, bool b_revrgb, bool b_pot ) {
      wxImage Image;
...
int s52plib::RenderToBufferAP( ObjRazRules *rzRules, Rules *rules, ViewPort *vp,
      ...
      RenderToBufferFilledPolygon( rzRules, rzRules->obj, NULL, vp->GetBBox(),
                  pb_spec, ppatt_spec );
...
int s52plib::RenderToBufferAC( ObjRazRules *rzRules, Rules *rules, ViewPort *vp,
      ...
      RenderToBufferFilledPolygon( rzRules, rzRules->obj, c, vp->GetBBox(),
                  pb_spec, NULL );
      ...
      if( vp->chart_scale > 5e7 ) {
            ...
            if( ( rzRules->obj->BBObj.GetMaxX() > vp->GetBBox().GetMinX() )
                  ...
                  //    If we add 360 to the objects lons, does it intersect the the right side of the VP?
                  if( ( ( rzRules->obj->BBObj.GetMaxX() + 360. )
                              > vp->GetBBox().GetMaxX() )
                              && ( ( rzRules->obj->BBObj.GetMinX() + 360. )
                                          < vp->GetBBox().GetMaxX() ) ) {
                        //  If so, this area oject should be drawn again, this time for the left side
                        //    Do this by temporarily adjusting the objects rendering offset
                        rzRules->obj->x_origin -= mercator_k0
                                    * WGS84_semimajor_axis_meters * 2.0 * PI;
                        RenderToBufferFilledPolygon( rzRules, rzRules->obj, c,
                                    vp->GetBBox(), pb_spec, NULL );
                        rzRules->obj->x_origin += mercator_k0
                                    * WGS84_semimajor_axis_meters * 2.0 * PI;
...
//  Soundings override
      if( !strncmp( rzRules->LUP->OBCL, "SOUNDG", 6 ) ) b_catfilter =
                  m_bShowSoundg;
...
//      SCAMIN Filtering
            ...
            if( m_bUseSCAMIN ) {
                  if( ( DISPLAYBASE == rzRules->LUP->DISC )
                              || ( PRIO_GROUP1 == rzRules->LUP->DPRI ) ) b_visible = true;
                  else if( vp->chart_scale > rzRules->obj->Scamin ) b_visible = false;
                  ...
                  if( ( !strncmp( rzRules->LUP->OBCL, "$TEXTS", 6 ) )
                              && ( vp->chart_scale > rzRules->obj->Scamin ) ) b_visible =
                              false;
            }
...
TexFont *txfLoadFont( char *filename ) {
      ...
      txf->tgi = (TexGlyphInfo *) malloc(
                  txf->num_glyphs * sizeof(TexGlyphInfo) );
      if( txf->tgi == NULL ) {
            lastError = (char *) "out of memory.";
            goto error;
      ...
      txf->tgvi = (TexGlyphVertexInfo *) malloc(
                  txf->num_glyphs * sizeof(TexGlyphVertexInfo) );
      if( txf->tgvi == NULL ) {
            lastError = (char *) "out of memory.";
            goto error;
      }
      ...
      txf->lut = (TexGlyphVertexInfo **) calloc( txf->range,
      ...
      switch ( format ){
            case TXF_FORMAT_BYTE:
                  if( useLuminanceAlpha ) {
                        ...
                        got = fread( txf->teximage, 1, txf->tex_width * txf->tex_height,
                                    file );
                        EXPECT( txf->tex_width * txf->tex_height );
                  }
                  break;
            case TXF_FORMAT_BITMAP:
                  ...
                  if( useLuminanceAlpha ) {
                        txf->teximage = (unsigned char *) calloc( width * height * 2,
                                    1 );
                        if( txf->teximage == NULL ) {
                              lastError = (char *) "out of memory.";
                              goto error;
      ...
      if( useLuminanceAlpha ) {
            if( setupMipmaps ) {
                  gluBuild2DMipmaps( GL_TEXTURE_2D, GL_LUMINANCE_ALPHA,
                              txf->tex_width, txf->tex_height, GL_LUMINANCE_ALPHA,
                              GL_UNSIGNED_BYTE, txf->teximage );
            } else {
...
void RenderFromHPGL::Circle( wxPoint center, int radius, bool filled ) {
      if( renderToDC ) {
            ...
            else targetDC->SetBrush( *wxTRANSPARENT_BRUSH );
      ...
      if( renderToOpenGl ) {
            ...
            for( double a = 0; a <= 2 * M_PI; a += 2 * M_PI / noSegments )
                  glVertex2d( center.x + radius * sin( a ),
